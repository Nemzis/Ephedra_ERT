...
class Komarov_SP:
    def __init__(self, parent, ui):
        ...
        self.rho_SP.set(self.state.get('check3', 0))  # По умолчанию 0, если ключа нет
    ...
    def create_komarov_tab(self):
        ...
        open_button_safe_chek = ttk.Button(self.komarov_body_tab, width = w, text='Cохранить настройки', command=self.on_closing)
        ...
        self.label_text_info = tk.Label(self.komarov_body_tab, text='Формула рассчета: (A-B)*100/B')
        ...
        checkbutton_100 = tk.Checkbutton(
            ...
            text='+100 к рассчетным данным (при нажатии "Рассчитать")',
            variable=self.sp100
        )
        ...
        checkbutton_rho_SP = tk.Checkbutton(
            self.komarov_body_tab,
            text='Заменить КС значением ВП (при нажатии "Рассчитать")',
            variable=self.rho_SP
        )
        
        checkbutton_rho_SP.grid(row=a, column=0, padx=0, pady=0, columnspan=50, sticky='nw')
        a += 1
    ...
    def array_low_load(self):
        ...
        try:
            if self.filepath_1 != self.filepath_2:
                ...
                self.label_text_load_1['text'] = (f'{self.filepath_1}: строчек - {len(self.array_low)}')
    ...
    def array_high_load(self):
        ...
        try:
            if self.filepath_1 != self.filepath_2:
                ...
                self.label_text_load_2['text'] = (f'{self.filepath_2}: строчек - {len(self.array_high)}')
    ...
    def rok_minus_rok_2(self):
        ...
        if self.file_extension_low == self.file_extension_high:
                        ...
                        for item2 in self.array_high_copy:
                            if item1[2] == item2[2] and\
                                ...
                                item1[16] == item2[16]:
                                    ...
                                    if self.rho_SP.get() == 1:
                                        item1[1] = (item1[1] - item2[1]) * (100 / item2[1]) + plus
                                    else:
                                        result_t = (item1[1] - item2[1]) * (100 / item2[1]) + plus
                                        item1.append(result_t)
                            ...
                            if item1[23] == item2[23] and\
                                ...
                                item1[36] == item2[36]:
                                    ...
                                    if self.rho_SP.get() == 1:
                                        ...
                                        result_y = (item1[1] - item2[1]) * (100 / item2[1]) + plus
                                        item1.append(result_y)
                        ...
                        for item2 in self.array_high_copy:
                            if item1[0] == item2[0] and\
                                ...
                                item1[6] == item2[6]:
                                    ...
                                    if self.rho_SP.get() == 1:
                                        item1[7] = ((item1[7] - item2[7]) * (100 / item2[7])) + plus
                                    else:
                                        result_v = ((item1[7] - item2[7]) * (100 / item2[7])) + plus
                                        item1.append(result_v)                                    
                        ...
                        for item2 in self.array_high_copy:
                            if item1[0] == item2[0] and\
                                ...
                                item1[8] == item2[8]:
                                    ...
                                    if self.rho_SP.get() == 1:
                                        item1[9] = ((item1[9] - item2[9]) * (100 / item2[9])) + plus
                                    else:
                                        result_s = ((item1[9] - item2[9]) * (100 / item2[9])) + plus
                                        item1.append(result_s)
            ...
            if len(self.result_data) == 0:
                ...
                self.ui.update_message(f'Разница входного и выходного файла низкой частоты - {a}')
                self.ui.update_message(f'Разница входного и выходного файла высокой частоты - {b}')

                self.label_text_result['text'] = (f'Рассчет выполнен, строчек: {len(self.result_data)}')
    ...
    def save_state(self, state):
        ...
        try:
            settings_path = get_path("module", "komarov_sp", "settings.json")
    ...
    def on_closing(self):
        ...
        self.state = {
            'check2': self.sp100.get(),
            'check3': self.rho_SP.get()
        }
    ...
    def helper(self):
        with open(
            get_path("module", "komarov_sp", "helper_Komarov.txt"),
            "r",
            encoding="utf-8"
        ) as file:
